#@title /content/dreamtalk/core/utils.py
# %%writefile /content/dreamtalk/core/utils.py
import os
import argparse
...
def write_json(image_path, first_bbox,bbox,height, width):
    if not os.path.exists("./upload_images"):
        os.mkdir("./upload_images")

    file_name = os.path.basename(image_path)
    new_path = f"{os.getcwd()}/upload_images/{file_name}"
...
#         write_json(src_img,first_bbox, bbox,height, width)
#     else:
...
def getFaceAnalyser(det_size=(320, 320)):
    ...
    face_analyser = insightface.app.FaceAnalysis(
        name="buffalo_l", root=f"{os.getcwd()}/checkpoints", providers=providers
    )
    print("Available Providers after:", onnxruntime.get_available_providers())
    face_analyser.prepare(ctx_id=0, det_size=det_size)
    return face_analyser
def get_one_face(img):
    face_analyser=getFaceAnalyser()
    face = face_analyser.get(img)
    try:
        first_face=min(face, key=lambda x: x.bbox[0])
        bbox=[]
        for i in first_face['bbox']:
          bbox.append(int(i))
        return bbox
    except ValueError:
        return None
...
def crop_src_image(src_img, save_img, increase_ratio, detector=None):
    ...
    if bbox is not None:
        ...
        bbox[1] = max(0, bbox[1])
        bbox[3] = min(h, bbox[3])
        #modified increase_ratio we need more box area
        increase_ratio=2
        bbox = compute_aspect_preserved_bbox(
            tuple(bbox), increase_ratio, img.shape[0], img.shape[1]
        )
